...
- `Warning` (string, optional): set when the compile was requested during Play Mode (the Play session is discarded), while pause points were armed in either Play Mode or Edit Mode (the domain reload drops every pause point patch, except those enabled with `--persist`, which are counted separately and re-armed after the reload), or while hot-reload changes were active (a successful compile drops every patch; the edited sources are compiled in).
- `Message`: string
